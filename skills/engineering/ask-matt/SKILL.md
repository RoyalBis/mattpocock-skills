...
- **`/grill-me`** — the same focused questioning as `/grill-with-docs`, but **stateless**: it saves nothing locally and builds no `CONTEXT.md`. Reach for it when you are **not working in a working directory** — sharpening a plan, a design, a piece of writing, anything with no repo under it. If you are in a working directory, use `/grill-with-docs` instead: it runs the same discussion and leaves a paper trail, so it is strictly the better one.
...
- **`/validate-mermaid`** — check Mermaid syntax in Markdown, MDX, HTML, or standalone diagram files. It can scan a directory recursively and uses Mermaid's own parser at the renderer's version. Reach for it after generating diagrams or when a browser reports `Syntax error in text`; it does not replace a browser check for layout.
- **`/slidev`** — build or revise a Slidev presentation as a staged argument: one beat per slide, a small set of tracer slides to prove the visual system, speaker notes for the delivery path, then a rendered review of every slide and meaningful click state. User-invoked only; run it explicitly against an existing deck, or explicitly ask it to create a new one.
