...
Every `SKILL.md` is either user-invoked (`policy.allow_implicit_invocation: false` in `agents/openai.yaml`, reachable only by the human) or model-invoked (model- or user-reachable). Keep invocation policy out of `SKILL.md` frontmatter so skills remain Codex-valid. See [.agents/invocation.md](./.agents/invocation.md).
