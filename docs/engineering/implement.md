...
You invoke this by typing `/implement` — the agent won't reach for it on its own. Its `agents/openai.yaml` sets `policy.allow_implicit_invocation: false`, so wherever [ask-matt](https://aihero.dev/skills-ask-matt) or [to-tickets](https://aihero.dev/skills-to-tickets) says "then `/implement` per ticket", that is an instruction to you, not something the agent will do unprompted.
