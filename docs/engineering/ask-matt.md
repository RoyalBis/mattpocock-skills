...
| Mermaid source that needs checking before publication | [validate-mermaid](https://aihero.dev/skills-validate-mermaid), including recursive scans across generated documentation |
...
A known bug, partly mitigated by keeping user-invoked policy in `agents/openai.yaml` instead of `SKILL.md` frontmatter. Some harnesses still hide user-invoked skills from the model's automatic skill list, and the agent may read that list as exhaustive. They are installed. Type the slash command anyway, or check `.claude-plugin/plugin.json`, which is the authority on what the plugin ships.
